...
var jdd = {
    ...
    generateReport: function () {
        ...
        var filterBlock = $('<span class="filterBlock"></span>');
    ...
    compareJson: function (jsonLeft, jsonRight, prefix) {
        ...
        var diffs = "";
        if (jdd.diffs.length > 0) {
            diffs += prefix + '变更 <font color=red>' + (jdd.diffs.length) + '</font> 处';
        }
        var missing = "";
        if (missingCount > 0) {
            missing += '删除 <font color=red>' + missingCount + '</font> 个属性 ';
        }
        var types = "";
        if (typeCount > 0) {
            types += '产生 <font color=red>' + typeCount + '</font> 个错误类型 ';
        }
        var eq = "";
        if (eqCount > 0) {
            eq += '修改 <font color=red>' + eqCount + '</font> 个属性值';
        }

        if (jdd.diffs.length > 0) {
            return diffs + "( " + missing + types + eq + " )<br/>";
        } else {
            return "";
        }
    },
...
jQuery(document).ready(function () {
    ...
    if ($('#textarealeft').val() && $('#textarearight').val()) {
        jdd.compare();
    }
